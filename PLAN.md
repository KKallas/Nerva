...
## 1. The core idea: build a list, then file it

Everything you do in the lab is the same two moves:
...
| URL                  | Who   | What                                                        |
|----------------------|-------|-------------------------------------------------------------|
| `/`                  | all   | **The list.** Big textarea, scan button (camera overlay, each scan appends a line), verb buttons `find out in count new`, results under each line after filing. |
...
1. **List + find** – Express, JSON store with atomic writes, the list page with textarea and `find`, item page, search. No identity needed yet. Usable on day one for "where is it".
