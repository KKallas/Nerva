...
- Photos go through `Nerva.photoFromCamera()`: pick, shrink, then the finger-paint highlighter in `public/paint.js`. The highlight is flattened into the JPEG, never stored separately. Its edge is soft on purpose: strokes go on a `reveal` layer, which is blurred and wiped out of the dark layer several times so the middle clears fully while the edge fades. Redraw each stroke from its own snapshot, or overlapping dabs harden the edge.
