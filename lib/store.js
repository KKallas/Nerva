...
class Store {
  ...
  // Items filed at this place. A location includes everything on its shelves.
  itemsAt(placeId) {
    const id = String(placeId || '').toLowerCase();
    const prefix = id + '-';
    return [...this.items.values()].filter(i => i.shelf === id || (i.shelf || '').startsWith(prefix));
  }

  // Renaming a place rewrites the text denormalised onto its items, so search
  // and the catalogue never need to know that places are records.
  syncPlaceText(placeId) {
    let n = 0;
    for (const item of this.itemsAt(placeId)) {
      const text = this.placeText(item.shelf);
      if (text && item.location !== text) { item.location = text; this.saveItem(item); n++; }
    }
    return n;
  }
