...
class Store {
  constructor(dir) {
    ...
    // A lab has tens of people, not thousands: one file, keyed by user id.
    this.usersFile = path.join(dir, 'users.json');
    this.users = new Map(Object.entries(this.readJson(this.usersFile, {})));
  }

  saveUser(user) {
    this.users.set(user.id, user);
    writeAtomic(this.usersFile, JSON.stringify(Object.fromEntries(this.users), null, 2));
    return user;
  }
  ...
  // `out` says what is on loan and when it is due back, so a list being built
  // at the shelf can show it offline.
  catalogue() {
    const out = outIndex(this);
    return [...this.items.values()].map(i => ({
      ...
      consumable: i.consumable && !i.tracked ? true : undefined, out: out.get(i.id),
    }));
  }
}
